...
Commitments are `keccak256(abi.encode(proposalId, voter, choice, salt))`, with Solidity types `(uint256, address, bool, bytes32)`. This revision intentionally replaces the brief’s unbound `(choice, salt)` preimage to prevent copied ballots and cross-proposal replay. Use standard ABI encoding, not packed encoding. The voter must be the address that calls commit and reveal. Each address gets one commitment per proposal. Each phase must last at least one hour at proposal creation, and the reveal deadline must be no more than 90 days after creation. These immutable limits reject impractically short windows and bound the stake lock; they do not guarantee inclusion during a chain outage. The commit interval ends at `commitDeadline`; reveal is allowed from `commitDeadline` (inclusive) to `revealDeadline` (exclusive); reclaim begins at `revealDeadline`. Only revealed weight enters the immutable final totals. Non-revealers still recover their stake after the deadline, preventing permanent custody from a lost salt. Proposals are independent, so a voter must hold and lock separate tokens for overlapping proposals.
